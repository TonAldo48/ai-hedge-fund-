#!/usr/bin/env python3
"""
Test script for michael_burry agent with LangSmith tracing.
"""
...
def test_michael_burry_with_tracing():
    """Test michael_burry agent with tracing enabled."""
    
    print("🚀 Testing Michael Burry Agent with LangSmith Tracing")
    print("=" * 60)
    ...
    test_ticker = "GME"  # Use GameStop as test case (contrarian stock)
    ...
    state = AgentState(
        ...
        session_id=f"test_michael_burry_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    print(f"   💭 Investment Style: deep_value_contrarian")
    
    # Run michael_burry analysis
    print(f"\n3. Running Michael Burry analysis for {test_ticker}...")
    try:
        ...
        # Display results
        if "analyst_signals" in result["data"] and "michael_burry_agent" in result["data"]["analyst_signals"]:
            analysis = result["data"]["analyst_signals"]["michael_burry_agent"][test_ticker]
            ...
            if get_tracing_enabled():
                ...
                print(f"   Investment Style: deep_value_contrarian")
                print(f"   Key Metrics: FCF yield, EV/EBIT, debt/equity, insider activity, contrarian sentiment")
...
if __name__ == "__main__":
    test_michael_burry_with_tracing() 