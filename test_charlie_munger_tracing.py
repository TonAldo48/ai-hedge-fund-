#!/usr/bin/env python3
"""
Test script for charlie_munger agent with LangSmith tracing.
"""
...
def test_charlie_munger_with_tracing():
    """Test charlie_munger agent with tracing enabled."""
    
    print("🚀 Testing Charlie Munger Agent with LangSmith Tracing")
    print("=" * 60)
    ...
    state = AgentState(
        ...
        session_id=f"test_charlie_munger_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
    ...
    # Run charlie_munger analysis
    print(f"\n3. Running Charlie Munger analysis for {test_ticker}...")
    try:
        result = charlie_munger_agent(state)
        
        print("✅ Analysis completed successfully!")
        
        # Display results
        if "analyst_signals" in result["data"] and "charlie_munger_agent" in result["data"]["analyst_signals"]:
            analysis = result["data"]["analyst_signals"]["charlie_munger_agent"][test_ticker]
...
if __name__ == "__main__":
    test_charlie_munger_with_tracing()
